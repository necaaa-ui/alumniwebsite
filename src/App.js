...
function App() {
  ...
  if (email) {
    setLoading(true);
    axios
      .get(`http://localhost:5000/api/user-by-email`, {
        params: { email },
      })
      .then((res) => {
        const user = res.data;
      ...
      .catch((err) => {
        console.error('User not found in form API:', err);
        setLoading(false);
      });
  } else {
    setLoading(false);
  ...
  const handleCaptchaChange = async (value) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/verifycaptcha', {
        value,
      });
      ...
      setCaptchaVerified(false);
      alert('Error verifying CAPTCHA. Please check your network and try again.');
    }
  };
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !/^[A-Za-z\s,]+$/.test(formData.location)
    ) {
    ...
    if (
      !formData.skillset ||
      formData.skillset.length < 3 ||
      formData.skillset.length > 500
    ) {
    ...
    if (
      ...
      newErrors.message =
        'Please write a short message explaining why you are interested.';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setIsSubmitting(false);
      console.log('isSubmitting set to false after error');
    }
  };
